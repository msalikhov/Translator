...
class TranslationHistoryInterator @Inject constructor(
    private val translationRepository: TranslationRepository
) {

    fun observeHistoryModels(): Observable<List<TranslationHistoryModel>> = translationRepository
    ...
    fun toggleFavouriteItem(model: TranslationHistoryModel): Completable = if (model.isFavourite) {
        translationRepository.removeTranslationFromFavourites(model.translationId)
    } else {
        translationRepository.addTranslationToFavourites(FavouriteTranslationDBModel(translationId = model.translationId))
    }
}